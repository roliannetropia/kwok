...
create or replace view attribute_value_view as 
	select a.attribute_id, a.object_type_id, a.attribute_key as attribute_name, av.attr_value, av.object_id, av.attribute_field_id
...
create or replace view contract_view as 
	select c.contract_id, c.contract_name, c.contract_description, c.contract_owner as contract_owner_id, co.display_name as contract_owner_display_name, co.username as contract_owner_username, 
		c.contract_type, c.contract_effective_date, c.contract_expiration_date, c.contract_renewal_type, c.contract_renewal_date, 
...
-- 
create or replace function sp_attribute_add(out o_attribute_id int, in p_object_type_id int, in p_attribute_name varchar(50), in p_attribute_type int, in p_attribute_option text, in p_attribute_convert_url int, in p_attribute_url text, in p_default_attribute_field_id int, in p_attribute_group_id int)
RETURNS integer AS $$
begin
	select nextval('seq_attribute_id') into o_attribute_id;

	insert into attribute (attribute_id, object_type_id, attribute_key, attribute_url, attribute_type, attribute_option, attribute_convert_url, is_editable, default_attribute_field_id, attribute_group_id, is_custom_attr)
		values (o_attribute_id, p_object_type_id, p_attribute_name, p_attribute_url, p_attribute_type, p_attribute_option, p_attribute_convert_url, 1, p_default_attribute_field_id, p_attribute_group_id, 1);
end;
$$ LANGUAGE 'plpgsql';
...
-- 
create or replace function sp_system_attribute_update(in p_attribute_id int, in p_is_required int)
RETURNS void AS $$
begin
...
-- 
create or replace function sp_attribute_update(in p_attribute_id int, in p_attribute_name varchar(50), in p_attribute_type int, in p_attribute_option text, in p_attribute_convert_url int, in p_attribute_url text, in p_attribute_group_id int)
RETURNS void AS $$
begin
	update attribute set attribute_key = p_attribute_name, attribute_type=p_attribute_type, attribute_url=p_attribute_url,
		attribute_convert_url=p_attribute_convert_url,
		attribute_option=p_attribute_option, attribute_group_id=p_attribute_group_id
...
begin
	...
	delete from asset_hardware_component where hardware_id = p_hardware_id;
...
--
create or replace function sp_hardware_component_delete(in p_hardware_id int, in p_comp_id int)
RETURNS void AS $$
begin
...
--
create or replace function sp_contract_add(out o_contract_id int, in p_contract_name varchar(100), in p_contract_description text, in p_contract_type int, in p_contract_effective_date varchar, in p_contract_expiration_date varchar, in p_contract_renewal_date varchar, in p_contract_renewal_type int, in p_contract_provider_id int, in p_contract_owner int, in p_contract_provider_contact int, in p_creator int)
RETURNS integer AS $$
begin
	select nextval('seq_contract_id') into o_contract_id;

	insert into contract(contract_id, contract_name, contract_description, contract_type, contract_effective_date, contract_expiration_date, contract_renewal_date, contract_renewal_type, contract_provider, contract_owner, contract_provider_contact, creator, creation_date) 
		values (o_contract_id, p_contract_name, p_contract_description, p_contract_type, to_timestamp(p_contract_effective_date, 'YYYY-MM-DD HH24:MI:SS'), to_timestamp(p_contract_expiration_date, 'YYYY-MM-DD HH24:MI:SS'), to_timestamp(p_contract_renewal_date, 'YYYY-MM-DD HH24:MI:SS'), p_contract_renewal_type, p_contract_provider_id, p_contract_owner, p_contract_provider_contact, p_creator, now());
end;
$$ LANGUAGE 'plpgsql';
...
--
create or replace function sp_contract_update(in p_contract_id int, in p_contract_name varchar(100), in p_contract_description text, in p_contract_type int, in p_contract_effective_date varchar, in p_contract_expiration_date varchar, in p_contract_renewal_date varchar, in p_contract_renewal_type int, in p_contract_provider_id int, in p_contract_owner int, in p_contract_provider_contact int, in p_modifier int)
RETURNS void AS $$
begin
	update contract set contract_name=p_contract_name, contract_description=p_contract_description, contract_type=p_contract_type, contract_effective_date=to_timestamp(p_contract_effective_date, 'YYYY-MM-DD HH24:MI:SS'),
...
--
create or replace function sp_issue_update(in p_issue_id int, in p_issue_name varchar(100), in p_issue_description text, in p_issue_type int, in p_issue_status int, in p_issue_priority int, in p_issue_resolution int, in p_issue_assignee int, in p_issue_due_date varchar, in p_issue_from_email varchar(225), in p_user_id int)
RETURNS void AS $$
...
begin	
	...
	perform sp_file_delete(p_object_type_id, p_object_id);
